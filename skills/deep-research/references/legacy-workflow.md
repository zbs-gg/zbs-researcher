# Legacy research modes and configuration reference

This file is loaded ONLY for an explicit legacy-mode request or old run.
For new full research, use goal-driven.md instead and do not run this intake.
Any paid execution still requires the user's explicit known budget; key presence
alone is never authorization, regardless of historical wording below.
...
3. **Pick channels + aim each one.** Decide which of the 19 connectors run
...
odds, velocity; mostly free, with paid/credential gates named explicitly):
...
| **x** *(opt-in, off by default)* | Monid-discovered allowlisted provider (pay-per-use) | public X posts with author, date, engagement, and canonical link; route and quoted unit price are shown before the paid run |
| **bluesky** *(opt-in, off by default)* | app.bsky searchPosts | top posts *(best-effort)* |
...
- **`entity-fanout`** — actual deep research. It (1) **enumerates the top-N
  ...
  **Hybrid tiering (the default):** the free channels (hackernews, github-issues,
  reddit) run on **all N** entities; the paid LLM lenses (grok, gemini,
...
live X via Grok's `x_search` or explicit direct-post retrieval through Monid,
Telegram communities through a real client
...
| **x** (direct public posts) | one narrow phrase, exact @handle, or product name; explicit Monid spend |
...
**github-issues** (repo-scoped: the actual open wounds) and **X via grok** or
the explicit direct `x` connector
(who is complaining right now, in their own words). Bounds:

- **4 rounds max** by default; stop earlier the moment a round surfaces
  no new leads.
- **Paid-source budget:** grok / gemini / perplexity fires bill real vendor
  tokens — keep them for the leads that matter, not for round-one carpet
  coverage. `x` bills Monid per call/result and must be explicitly selected;
  it is never a fallback. The free channels (hackernews, github,
  github-issues, reddit) carry the breadth.
...
threads, monid}, telegram_session, profile, wizard_done, tier, persona,
...
`--only hackernews,hiring,polymarket,github,github-issues,reddit` — on a topic the
...
python3 "$SKILL_DIR/scripts/deep-research.py" "<topic>" \
    --only hackernews,hiring,polymarket,github,github-issues,reddit
```
...
- **Direct public X via Monid** (opt-in, pay-per-use): `MONID_API_KEY` or
  `monid-key.txt` / `monid-api-key.txt`. Select only with `--fire x` or
  `--only x`. Discovery and inspection are free; the runner announces the
  allowlisted underlying route and quoted unit price before the paid run.
  Actual cost is claimed only when Monid returns billing evidence. A failed
  `grok` call never activates this route. SuperGrok/X Premium subscriptions
  can be used through xAI OAuth in supported agent clients, but Researcher's
  `GROK_API_KEY` path remains the separately managed API-team route; do not
  tell the user that a subscription automatically funds this key.
...
python3 "$SCRIPT" "TOPIC" --output-dir ./scratch/research-run --skip reddit
...
├── x.md                  — direct public X posts via Monid (opt-in, paid)
...
- Keys read from `~/.config/zbs-researcher/secrets/{gemini-key,grok-api-key,monid-key,openai-api-key,perplexity-key}.txt` (or env `GEMINI_API_KEY`, `GROK_API_KEY`, `MONID_API_KEY`, `OPENAI_API_KEY`, `PERPLEXITY_API_KEY`). Monid also accepts `monid-api-key.txt`; the OpenAI read is name-tolerant (`openai-api-key.txt` / `openai-key.txt` / `openai.txt`).
...
- Default direct channels are free and need no key. `x`, `tiktok-ig`, and some
  `threads` routes are explicit paid exceptions. `github` and `github-issues`
  prefer authed `gh` (higher rate limit), then fall back to unauthenticated API.
...
- **Reddit / Bluesky** are best-effort — Reddit may degrade to ERROR.md; Bluesky is available only by explicit selection and no longer consumes default-run work.
...
- `x` is off by default and bills Monid only when explicitly selected. Its
  catalog price is a quote, not an actual charge, unless the response includes
