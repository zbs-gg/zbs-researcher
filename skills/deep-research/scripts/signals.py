...
def signal_message(kind, notified):
    ...
    if notified:
        head = f"{subject} was recorded and the maintainer was notified."
    else:
        head = (
            f"{subject} was recorded locally in {SIGNALS_FILENAME} — "
            "no notification was sent; the maintainer will see it when this install syncs."
        )
    if kind == "host-for-me":
        return (
            head
            + " Nothing has been provisioned or billed — the maintainer will follow up"
            " before anything changes."
        )
    return head + " Nothing changes until the maintainer follows up — this only measures demand."
